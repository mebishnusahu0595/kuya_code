...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/anomalyco/kuyacode/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/kuyacode packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/kuyacode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/kuyacode/")) areas.add("core")
...
async function summarizeCommit(kuyacode: Awaited<ReturnType<typeof createKuyaCode>>, message: string): Promise<string> {
  ...
  const result = await kuyacode.client.session
    .prompt({
      ...
      body: {
        model: { providerID: "kuyacode", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], kuyacode: Awaited<ReturnType<typeof createKuyaCode>>) {
  ...
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(batch.map((c) => summarizeCommit(kuyacode, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/anomalyco/kuyacode/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  try {
    const lines = await generateChangelog(commits, kuyacode)
    notes.push(...lines)
