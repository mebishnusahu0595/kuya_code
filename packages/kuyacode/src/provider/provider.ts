...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async kuyacode(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["kuyacode"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kuyacode.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://kuyacode.ai/",
            "X-Title": "kuyacode",
          },
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "kuyacode",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    if (database["kuyacode"]) database["kuyacode"].name = "KuyaCode Zen"
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.KUYACODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("kuyacode")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if kuyacode provider is available before using it
    const kuyacodeProvider = await state().then((state) => state.providers["kuyacode"])
    if (kuyacodeProvider && kuyacodeProvider.models["gpt-5-nano"]) {
      return getModel("kuyacode", "gpt-5-nano")
    }
