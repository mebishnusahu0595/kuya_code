...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kuyacode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kuyacode sessions
...
kuyacode acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via kuyacode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kuyacode acp
```
...
ACP sessions map cleanly to kuyacode's internal session model:
