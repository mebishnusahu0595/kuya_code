...
import type { KuyaCodeClient, SessionMessageResponse } from "@kuyacode-ai/sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: KuyaCodeClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `kuyacode auth login` in the terminal",
        name: "Login with kuyacode",
        id: "kuyacode-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const kuyacodeProvider = providers.find((p) => p.id === "kuyacode")
    if (kuyacodeProvider) {
      if (kuyacodeProvider.models["big-pickle"]) {
        return { providerID: "kuyacode", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(kuyacodeProvider.models))
      if (best) {
        return {
