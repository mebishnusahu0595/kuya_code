...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run kuyacode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for kuyacode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kuyacode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`kuyacode import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting kuyacode...")
        UI.println()

        // Launch kuyacode TUI with session ID if available
        const { spawn } = await import("child_process")
        const kuyacodeArgs = sessionId ? ["-s", sessionId] : []
        const kuyacodeProcess = spawn("kuyacode", kuyacodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          kuyacodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`kuyacode exited with code ${code}`))
          })
          kuyacodeProcess.on("error", reject)
        })
      },
    })
  },
