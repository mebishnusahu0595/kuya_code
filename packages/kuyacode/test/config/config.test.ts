...
test("loads JSON config file", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "kuyacode.json"),
        JSON.stringify({
          $schema: "https://kuyacode.ai/config.json",
          model: "test/model",
          username: "testuser",
        }),
      )
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "kuyacode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kuyacode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kuyacodeDir = path.join(dir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })
      const agentDir = path.join(kuyacodeDir, "agent")
...
test("loads agents from .kuyacode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kuyacodeDir = path.join(dir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })

      const agentsDir = path.join(kuyacodeDir, "agents")
...
test("loads commands from .kuyacode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kuyacodeDir = path.join(dir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })

      const commandDir = path.join(kuyacodeDir, "command")
...
test("loads commands from .kuyacode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kuyacodeDir = path.join(dir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })

      const commandsDir = path.join(kuyacodeDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "kuyacode.json"),
        JSON.stringify({ $schema: "https://kuyacode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "kuyacode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kuyacode config
      const projectDir = path.join(dir, "project")
      const kuyacodeDir = path.join(projectDir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })
      ...
      // Local .kuyacode config with different plugins
      await Bun.write(
        path.join(kuyacodeDir, "kuyacode.json"),
        JSON.stringify({
          $schema: "https://kuyacode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kuyacode config with some overlapping plugins
      await Bun.write(
        path.join(kuyacodeDir, "kuyacode.json"),
        JSON.stringify({
          $schema: "https://kuyacode.ai/config.json",
...
test("local .kuyacode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kuyacode directory config enables it
      const kuyacodeDir = path.join(dir, ".kuyacode")
      await fs.mkdir(kuyacodeDir, { recursive: true })
      await Bun.write(
        path.join(kuyacodeDir, "kuyacode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kuyacode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/kuyacode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-kuyacode@2.4.3")).toBe("oh-my-kuyacode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@kuyacode/plugin@2.0.0")).toBe("@kuyacode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-kuyacode@2.4.3", "file:///project/.kuyacode/plugin/oh-my-kuyacode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.kuyacode/plugin/oh-my-kuyacode.js")
  })
  ...
  test("local plugin directory overrides global kuyacode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const kuyacodeDir = path.join(projectDir, ".kuyacode")
        const pluginDir = path.join(kuyacodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
