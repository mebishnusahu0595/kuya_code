...
import { createKuyaCode, type ToolPart } from "@kuyacode-ai/sdk"
...
console.log("🚀 Starting kuyacode server...")
const kuyacode = await createKuyaCode({
  port: 0,
})
console.log("✅ KuyaCode server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await kuyacode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kuyacode session...")
    const { client, server } = kuyacode
    ...
    console.log("✅ Created kuyacode session:", createResult.data.id)
  ...
  console.log("📝 Sending to kuyacode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 KuyaCode response:", JSON.stringify(result, null, 2))
