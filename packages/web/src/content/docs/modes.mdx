...
:::caution
Modes are now configured through the `agent` option in the kuyacode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in kuyacode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the kuyacode config.
...
kuyacode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.kuyacode/plans/*.md` to detail the plan itself
...
Configure modes in your `kuyacode.json` config file:
...
- Global: `~/.config/kuyacode/mode/`
- Project: `.kuyacode/mode/`

```markdown title="~/.config/kuyacode/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global kuyacode config and the project specific config.
...
Create mode files in `.kuyacode/mode/` for project-specific modes or `~/.config/kuyacode/mode/` for global modes:

```markdown title=".kuyacode/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/kuyacode/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
