...
To debug any issues with KuyaCode, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/kuyacode/log/`
- **Windows**: `%USERPROFILE%\.local\share\kuyacode\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `kuyacode --log-level DEBUG`.

---

### Storage

kuyacode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/kuyacode/`
- **Windows**: `%USERPROFILE%\.local\share\kuyacode`
...
If you're experiencing issues with KuyaCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/kuyacode/issues**](https://github.com/anomalyco/kuyacode/issues)
...
2. **Join our Discord**
   ...
   [**kuyacode.ai/discord**](https://kuyacode.ai/discord)
...
### KuyaCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `kuyacode upgrade`
...
To figure out what models you have access to, run `kuyacode models`
...
If you encounter API call errors, this may be due to outdated provider packages. kuyacode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/kuyacode
   ```

2. Restart kuyacode to reinstall the latest provider packages

This will force kuyacode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
kuyacode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
