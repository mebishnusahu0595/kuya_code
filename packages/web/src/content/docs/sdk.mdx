---
title: SDK
description: Type-safe JS client for kuyacode server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The kuyacode JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control kuyacode programmatically.
...
Create an instance of kuyacode:

```javascript
import { createKuyaCode } from "@kuyacode-ai/sdk"

const { client } = await createKuyaCode()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `kuyacode.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${kuyacode.server.url}`)
...
If you already have a running instance of kuyacode, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@kuyacode-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*kuyacode" },
})
