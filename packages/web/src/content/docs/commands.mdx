...
Create `.kuyacode/command/test.md`:

```md title=".kuyacode/command/test.md"
---
...
You can add custom commands through the KuyaCode config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your KuyaCode [config](/docs/config):

```json title="kuyacode.jsonc" {4-12}
{
...
- Global: `~/.config/kuyacode/command/`
- Per-project: `.kuyacode/command/`

```markdown title="~/.config/kuyacode/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".kuyacode/command/component.md"
---
...
```md title=".kuyacode/command/create-file.md"
---
...
```md title=".kuyacode/command/analyze-coverage.md"
---
...
```md title=".kuyacode/command/review-changes.md"
---
...
```md title=".kuyacode/command/review-component.md"
---
...
kuyacode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
