...
There are two types of agents in KuyaCode; primary agents and subagents.
...
KuyaCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
KuyaCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

KuyaCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `kuyacode.json` config file:
...
- Global: `~/.config/kuyacode/agent/`
- Per-project: `.kuyacode/agent/`

```markdown title="~/.config/kuyacode/agent/review.md"
---
...
If no temperature is specified, KuyaCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global KuyaCode config and the project specific config.
...
The model ID in your KuyaCode config uses the format `provider/model-id`. For example, if you're using [KuyaCode Zen](/docs/zen), you would use `kuyacode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="kuyacode.json" {3-6,9-12}
{
  "$schema": "https://kuyacode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="kuyacode.json" {3-5,8-10}
{
  "$schema": "https://kuyacode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `kuyacode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/kuyacode).
:::
...
```markdown title="~/.config/kuyacode/agent/docs-writer.md"
---
...
```markdown title="~/.config/kuyacode/agent/security-auditor.md"
---
