---
title: Server
description: Interact with kuyacode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `kuyacode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kuyacode client can use.
...
```bash
kuyacode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
kuyacode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `KUYACODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `kuyacode`, or set `KUYACODE_SERVER_USERNAME` to override it. This applies to both `kuyacode serve` and `kuyacode web`.

```bash
KUYACODE_SERVER_PASSWORD=your-password kuyacode serve
```
...
When you run `kuyacode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the kuyacode server to interact with kuyacode programmatically.
:::

This architecture lets kuyacode support multiple clients and allows you to interact with kuyacode programmatically.

You can run `kuyacode serve` to start a standalone server. If you have the
kuyacode TUI running, `kuyacode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the KuyaCode [IDE](/docs/ide) plugins.
...
The kuyacode server exposes the following APIs.
