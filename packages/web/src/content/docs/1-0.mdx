...
description: What's new in KuyaCode 1.0.
---

KuyaCode 1.0 is a complete rewrite of the TUI.
...
The new TUI works like the old one since it connects to the same kuyacode server.
...
version. However some older versions of KuyaCode always grab latest.

To upgrade manually, run

```bash
$ kuyacode upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ kuyacode upgrade 0.15.31
```
