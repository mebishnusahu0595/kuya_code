---
title: Providers
description: Using any LLM provider in KuyaCode.
---

import config from "../../../config.mjs"
export const console = config.console

KuyaCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your KuyaCode config.
...
in `~/.local/share/kuyacode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your KuyaCode
config.
...
KuyaCode Zen is a list of models provided by the KuyaCode team that have been
tested and verified to work well with KuyaCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with KuyaCode Zen.
:::

1. Run the `/connect` command in the TUI, select kuyacode, and head to [kuyacode.ai/auth](https://kuyacode.ai/auth).
...
It works like any other provider in KuyaCode and is completely optional to use.
...
To use Amazon Bedrock with KuyaCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running kuyacode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY kuyacode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile kuyacode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX kuyacode
   ```
   ...
   For project-specific or persistent configuration, use `kuyacode.json`:

   ```json title="kuyacode.json"
   {
     "$schema": "https://kuyacode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for kuyacode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX kuyacode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX kuyacode
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your kuyacode config.
...
```bash
GITLAB_INSTANCE_URL=https://gitlab.company.com GITLAB_TOKEN=glpat-xxxxxxxxxxxxxxxxxxxx kuyacode
```
...
Customize through `kuyacode.json`:
...
{
  "$schema": "https://kuyacode.ai/config.json",
  "plugin": ["@gitlab/kuyacode-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with kuyacode:
...
To use Google Vertex AI with KuyaCode:
...
2. Set the required environment variables:
   ...
   Set them while running kuyacode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id kuyacode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through kuyacode, you can always configure it yourself.
...
```jsonc title="~/.config/kuyacode/kuyacode.jsonc"
{
  "$schema": "https://kuyacode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [kuyacode-helicone-session](https://github.com/H2Shami/kuyacode-helicone-session) plugin to automatically log each KuyaCode conversation as a session in Helicone.

```bash
npm install -g kuyacode-helicone-session
```
...
```json title="kuyacode.json"
{
  "plugin": ["kuyacode-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each KuyaCode conversation listed as a separate session.
...
You can configure kuyacode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="kuyacode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://kuyacode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure kuyacode to use local models through LM Studio.

```json title="kuyacode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://kuyacode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure kuyacode to use local models through Ollama.

```json title="kuyacode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://kuyacode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with KuyaCode:
...
3. Copy the API key for use in KuyaCode.
...
6. **Important**: Before using cloud models in KuyaCode, you must pull the model information locally:
...
KuyaCode Zen is a list of tested and verified models provided by the KuyaCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>KuyaCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **KuyaCode Zen**.

   ```txt
   /connect
   ```

3. Enter your KuyaCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your kuyacode config.
...
5. You can also customize them through your kuyacode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' kuyacode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group kuyacode
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with KuyaCode:
...
You can also customize models through your kuyacode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with kuyacode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in kuyacode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `kuyacode.json` file in your project directory:

   ```json title="kuyacode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://kuyacode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="kuyacode.json" {9,11,17-20}
{
  "$schema": "https://kuyacode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow KuyaCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `kuyacode auth list` to see if the credentials
...
2. For custom providers, check the kuyacode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your kuyacode config.
