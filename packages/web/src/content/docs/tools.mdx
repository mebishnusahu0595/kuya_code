...
Tools allow the LLM to perform actions in your codebase. KuyaCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in KuyaCode.
...
:::note
This tool is only available when `KUYACODE_EXPERIMENTAL_LSP_TOOL=true` (or `KUYACODE_EXPERIMENTAL=true`).
:::
