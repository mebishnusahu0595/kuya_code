...
You can add external tools to KuyaCode using the _Model Context Protocol_, or MCP. KuyaCode supports both local and remote servers.
...
You can define MCP servers in your [KuyaCode Config](https://kuyacode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="kuyacode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/kuyacode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
KuyaCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, KuyaCode will:
...
If the server requires authentication, KuyaCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `kuyacode mcp auth <server-name>`.
...
```bash
kuyacode mcp auth my-oauth-server
```
...
```bash
kuyacode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, KuyaCode will store the tokens securely in `~/.local/share/kuyacode/mcp-auth.json`.
...
kuyacode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in KuyaCode, alongside built-in tools. So you can manage them through the KuyaCode config like any other tool.
...
```json title="kuyacode.json" {11, 14-18}
{
  "$schema": "https://kuyacode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
kuyacode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect KuyaCode to your Sentry account.
