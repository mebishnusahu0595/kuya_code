...
With KuyaCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, KuyaCode uses our own `kuyacode` theme.
...
KuyaCode comes with several built-in themes.
...
- Want KuyaCode to match their terminal's appearance
...
KuyaCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kuyacode/themes/*.json` or `$XDG_CONFIG_HOME/kuyacode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kuyacode/themes/*.json`
4. **Current working directory** - Defined in `./.kuyacode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kuyacode/themes
vim ~/.config/kuyacode/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .kuyacode/themes
vim .kuyacode/themes/my-theme.json
```
...
{
  "$schema": "https://kuyacode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
