...
const CLI_INSTALL_DIR: &str = ".kuyacode/bin";
const CLI_BINARY_NAME: &str = "kuyacode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("kuyacode-install.sh");
...
pub fn create_command(app: &tauri::AppHandle, args: &str) -> Command {
    ...
    return app
        .shell()
        .sidecar("kuyacode-cli")
        .unwrap()
        .args(args.split_whitespace())
        .env("KUYACODE_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("KUYACODE_CLIENT", "desktop")
