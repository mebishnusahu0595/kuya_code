...
const SETTINGS_STORE: &str = "kuyacode.settings.dat";
...
fn get_sidecar_port() -> u32 {
    option_env!("KUYACODE_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("KUYACODE_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, port: u32, password: &str) -> CommandChild {
    ...
    let (mut rx, child) = cli::create_command(app, format!("serve --port {port}").as_str())
        .env("KUYACODE_SERVER_PASSWORD", password)
        .spawn()
        .expect("Failed to spawn kuyacode");
...
async fn check_server_health(url: &str, password: Option<&str>) -> bool {
    ...
    if let Some(password) = password {
        req = req.basic_auth("kuyacode", Some(password));
    }
        ...
        .setup(move |app| {
            ...
            let window_builder = WebviewWindowBuilder::from_config(&app, config)
                ...
                .initialization_script(format!(
                    r#"
                      window.__KUYACODE__ ??= {{}};
                      window.__KUYACODE__.updaterEnabled = {updater_enabled};
                    "#
                ));
...
async fn spawn_local_server(
    ...
    loop {
        if timestamp.elapsed() > Duration::from_secs(30) {
            break Err(format!(
                "Failed to spawn KuyaCode Server. Logs:\n{}",
