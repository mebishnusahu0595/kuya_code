...
import { AppBaseProviders, AppInterface, PlatformProvider, Platform } from "@kuyacode-ai/app"
...
const createPlatform = (password: Accessor<string | null>): Platform => ({
    ...
    await Promise.resolve()
      .then(() => {
        const notification = new Notification(title, {
          body: description ?? "",
          icon: "https://kuyacode.ai/favicon-96x96.png",
        })
  ...
  fetch: (input, init) => {
    ...
    const addHeader = (headers: Headers, password: string) => {
      headers.append("Authorization", `Basic ${btoa(`kuyacode:${password}`)}`)
    }
      ...
      <AppBaseProviders>
        <ServerGate>
          {(data) => {
            ...
            window.__KUYACODE__ ??= {}
            window.__KUYACODE__.serverPassword = data().password ?? undefined
