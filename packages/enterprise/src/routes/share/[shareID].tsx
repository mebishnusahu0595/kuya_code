import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@kuyacode-ai/sdk/v2"
import { SessionTurn } from "@kuyacode-ai/ui/session-turn"
import { SessionReview } from "@kuyacode-ai/ui/session-review"
import { DataProvider } from "@kuyacode-ai/ui/context"
import { DiffComponentProvider } from "@kuyacode-ai/ui/context/diff"
import { CodeComponentProvider } from "@kuyacode-ai/ui/context/code"
import { WorkerPoolProvider } from "@kuyacode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@kuyacode-ai/ui/logo"
import { IconButton } from "@kuyacode-ai/ui/icon-button"
import { ProviderIcon } from "@kuyacode-ai/ui/provider-icon"
import { createDefaultOptions } from "@kuyacode-ai/ui/pierre"
import { iife } from "@kuyacode-ai/util/iife"
import { Binary } from "@kuyacode-ai/util/binary"
import { NamedError } from "@kuyacode-ai/util/error"
import { DateTime } from "luxon"
import { SessionMessageRail } from "@kuyacode-ai/ui/session-message-rail"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@kuyacode-ai/ui/tabs"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@kuyacode-ai/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@kuyacode-ai/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@kuyacode-ai/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@kuyacode-ai/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@kuyacode-ai/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/kuyacode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | KuyaCode</Title>
              </Show>
              <Meta name="description" content="kuyacode - The AI coding agent built for the terminal." />
                          ...
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              ...
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/anomalyco/kuyacode"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://kuyacode.ai/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
